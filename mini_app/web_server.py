...
from config import CORS_ORIGINS, LAUNDRY_DATA_PATH
...
async def login(req: LoginRequest):
    user = is_registered(req.telegram_id)
    if not user:
        raise HTTPException(status_code=404, detail="Пользователь не найден. Сначала зарегистрируйтесь в боте")
...
async def get_slots(date: str):
    ...
    for name in get_machine_names():
        ...
        for hour in range(0, 24):
            ...
            if (
                not available
                and date in schedule.schedule
                and mid in schedule.schedule[date]
            ):
        ...
        result[mid] = {"name": name, "slots": slots}
...
async def create_booking(
    req: MultiBookingRequest, user_id: int = Depends(get_current_user)
):
    ...
    for b in req.bookings:
        if not schedule.is_time_available(
            b.date, b.machine_id, b.start_time, b.end_time
        ):
            raise HTTPException(
                status_code=400, detail=f"Слот {b.start_time}-{b.end_time} занят"
            )
    ...
    if balance < total:
        raise HTTPException(
            status_code=400, detail=f"Недостаточно средств: нужно {total}₽"
        )
    ...
    for b in req.bookings:
        schedule.add_booking(
            b.date, b.machine_id, b.start_time, b.end_time, label, str(user_id)
        )
...
async def cancel_booking(req: CancelRequest, user_id: int = Depends(get_current_user)):
    ...
    ok = schedule.remove_booking(
        req.date, req.machine_id, req.start_time, req.end_time, str(user_id)
    )
    if not ok:

...
async def accepting_registration(call: CallbackQuery):
    ...
    await call.bot.send_message(
        user_id, "Вы успешно зарегистрированы!", reply_markup=get_main_menu_kb(user_id)
    )
...
async def declining_registration(call: CallbackQuery):
    ...
    try:
        ...
        await call.bot.send_message(
            user_id, "Неправильный формат!!!", reply_markup=get_start_kb()
        )
...
async def cmd_add_money(message: Message):
    ...
    if len(parts) != 3:
        await message.answer(
            "Формат: /addmoney user_id amount\nПример: /addmoney 123456789 500"
        )
        return
